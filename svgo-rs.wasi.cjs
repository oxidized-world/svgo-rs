// napi-rs-artifact-metadata:{"version":2,"rootEntry":"index.js","exports":["optimize"],"managedRootEntries":["browser.js","index.js","svgo-rs.wasm","svgo-rs.debug.wasm"]}
/* eslint-disable */
/* prettier-ignore */
...
let __wasmFilePath = __nodePath.join(__dirname, 'svgo-rs.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'svgo-rs.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@svg-rs/svgo-wasm32-wasi')
  const __packagedWasmFilePath = __nodePath.join(
    __nodePath.dirname(__wasiPackageEntry),
    'svgo-rs.wasm32-wasi.wasm',
  )
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@svg-rs/svgo-wasm32-wasi is installed but is missing svgo-rs.wasm32-wasi.wasm.',
    )
  }
